...
<body>
    <div class="container_zone">
        ...
        <!-- 个人中心 -->
        <div class="zone_pc zone_message">
            <div class="location">
                <i class="icon iconfont icon-llcouponmapmark"></i> 当前位置 > 个人中心 > 消息通知
            </div>
            <div class="message">
                <ul id="message_ul">
                	<c:forEach items="${messages}" var="c" varStatus="status">
	                    <li>
	                        <span class="order">${status.index+1}.</span>
	                        <span class="title">
	                            <a href="javascript:void(0);">${c.title}</a>
	                        </span>
	                        <c:if test="${c.isRead==1}">
	                            <span style="float:left;display:block;width:4px;height:4px;border:1px solid #F90E65;background-color:#F90E65;border-radius: 2px;" id="new"></span>
	                        </c:if>
	                        <div class="cz" style="float:right">
	                        	<a href="${pageContext.request.contextPath}/user/message/delete/${c.id}" class="delete" title="删除" onclick="return confirm('你确定要删除？')"></a>
	                        </div>
	                        <div class="content">
	                        	${c.content }
	                        </div>
	                        <input type="hidden" value="${ c.id}" id="messageId"/>
	                        <input type="hidden" value="${ c.isRead}" id="isRead"/>
	                    </li>
                	</c:forEach>
                </ul>
            </div>
